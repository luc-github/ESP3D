...
void lv_init(void) {
  ...
  if (txt_u8[0] != 0xc3 || txt_u8[1] != 0x81 || txt_u8[2] != 0x00) {
    LV_LOG_WARN(
        "The strings have no UTF-8 encoding. Non-ASCII characters won't be "
        "displayed.");
  }
  ...
  LV_LOG_WARN(
      "Memory integrity checks are enabled via LV_USE_ASSERT_MEM_INTEGRITY "
      "which makes LVGL much slower");
#endif

#if LV_USE_ASSERT_OBJ
  LV_LOG_WARN(
      "Object sanity checks are enabled via LV_USE_ASSERT_OBJ which makes LVGL "
      "much slower");
#endif
