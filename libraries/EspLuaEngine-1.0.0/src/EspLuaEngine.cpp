...
bool EspLuaEngine::registerFunction(const char* name, lua_CFunction function) {
    if (!_checkPreconditions(name) || !function) {
        log_e("Error: Invalid function pointer");
        return false;
    }
...
template<>
bool EspLuaEngine::registerConstant<unsigned char>(const char* name, unsigned char value) {
    return _registerConstantImpl(name, static_cast<int>(value));
}

// Specializations explicit
template bool EspLuaEngine::registerConstant<lua_Number>(const char*, lua_Number);
template bool EspLuaEngine::registerConstant<const char*>(const char*, const char*);
...
template bool EspLuaEngine::registerConstant<unsigned char>(const char*, unsigned char);
