  ...
  if (noError) {
    ...
    if (parameter.length() == 0) {
      response = format_response(
          COMMANDID, json, true,
          String(ESP3DSettings::read_uint32(ESP_WEBDAV_PORT)).c_str());
    } else {  // set
#ifdef AUTHENTICATION_FEATURE
      if (auth_type != admin) {
        response = format_response(COMMANDID, json, false,
      ...
      if (noError) {
        ...
        if (!ESP3DSettings::isValidIntegerSetting(ibuf, ESP_WEBDAV_PORT)) {
          response = format_response(COMMANDID, json, false, "Incorrect port");
          noError = false;
        } else {
          if (!ESP3DSettings::write_uint32(ESP_WEBDAV_PORT, ibuf)) {
