...
ESP_File ESP_FileSystem::open(const char *path, uint8_t mode) {
  ...
  if (((strcmp(path, "/") == 0) &&
      ...
      (strlen(path) == 0)) {
    esp3d_log("reject  %s", path);
    return ESP_File();
  }
  // path must start by '/'
  if (path[0] != '/') {
    esp3d_log("%s is invalid path", path);
    return ESP_File();
  }
...
ESP_File::ESP_File(void *handle, bool isdir, bool iswritemode,
  ...
  for (uint8_t i = 0; (i < ESP_MAX_OPENHANDLE) && !set; i++) {
    if (!tFile_handle[i]) {
      ...
      esp3d_log("File %d busy", i);
      esp3d_log("%s", tFile_handle[i].name());
    }
  }
  if (!set) {
  ...
  if (_index != -1) {
    ...
    if (_iswritemode && !_isdir) {
      ...
      if (ftmp) {
        ...
        esp3d_log("Error opening %s", _filename.c_str());
      }
    }
