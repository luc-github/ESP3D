...
ESP_File ESP_FileSystem::open(const char *path, uint8_t mode) {
  ...
  File ftmp = LittleFS.open(path, (mode == ESP_FILE_READ)    ? "r"
                                  : (mode == ESP_FILE_WRITE) ? "w"
                                                             : "a");
  if (ftmp) {
    esp3d_log("Success openening: %s", path);
    if (ftmp.isFile()) {
      esp3d_log("It is a file");
      ESP_File esptmp(&ftmp, false, (mode == ESP_FILE_READ) ? false : true,
                      path);
      return esptmp;
    }
    if (ftmp.isDirectory()) {
      esp3d_log("It is a Directory");
    }
    ftmp.close();
  }
  esp3d_log("Opening as Directory");
  ...
  while (pathlist.size() > 0) {
    ...
    while (dir.next()) {
      if (dir.isDirectory()) {
        ...
        esp3d_log("remove %s", filepath.c_str());
  ...
  if (_isdir) {
    for (uint8_t i = 0; (i < ESP_MAX_OPENHANDLE) && !set; i++) {
      if (tDir_handle[i].fileName().length() == 0) {
        ...
        esp3d_log("Dir: %s index: %d", _name.c_str(), _index);
...
void ESP_File::close() {
  if (_index != -1) {
    if (_isdir) {
      esp3d_log("Closing Dir at index %d", _index);
      tDir_handle[_index] = Dir();
      _index = -1;
      return;
    }
    esp3d_log("Closing File at index %d", _index);
    esp3d_log("Size is %d", tFile_handle[_index].size());
    tFile_handle[_index].close();
    // reopen if mode = write
    // udate size + date
    if (_iswritemode && !_isdir) {
      esp3d_log("Updating Size of %s", _filename.c_str());
      File ftmp = LittleFS.open(_filename.c_str(), "r");
      if (ftmp) {
        _size = ftmp.size();
        esp3d_log("Updating Size to %d", ftmp.size());
...
ESP_File ESP_File::openNextFile() {
  ...
  if (tDir_handle[_index].next()) {
    ...
    esp3d_log("Getting next file from %s", _filename.c_str());
    esp3d_log("name :%s %s", name.c_str(),
              (tDir_handle[_index].isDirectory()) ? "isDir" : "isFile");
    String s = _filename;
    if (s[s.length() - 1] != '/') {
      s += "/";
    }
    ...
    if (tDir_handle[_index].isFile()) {
      ESP_File esptmp(name.c_str(), s.c_str(), false,
                      tDir_handle[_index].fileSize());
      return esptmp;
    } else {
      esp3d_log("Found dir  name: %s filename:%s", name.c_str(), s.c_str());
