...
/**
 * If the display is started, set the status to blank, set the started flag to
 * false, set the screen ID to the splash screen, set the splash displayed flag
 * to false, and clear the screen
 *
 */
void Display::end() {
  if (!_started) {
...
const char *Display::getModelString() { return "TFT_SPI_ST7789_135X240"; }

uint8_t Display::getModelID() { return DISPLAY_DEVICE; }
...
/**
 * The function takes a string as an argument and sets the value of the private
 * member variable _status to the value of the argument
 *
 * @param status The status to set.
 */
void Display::setStatus(const char *status) { _status = status; }

/**
 * The function clears the screen by calling the clear() function of the
 * esp3d_screen object
 */
...
/**
 * It takes a string and a maximum width, and returns the number of characters
 * that will fit in that width
 *
...
/**
 * The function is called with a value between 0 and 100. The function draws a
 * rectangle on the screen with a width that is proportional to the value
 *
...
void Display::progress(uint8_t v) {
  ...
  // display bar
  fillRect(10, _screenHeight - 4, ((_screenWidth - 20) * v) / 100, 4,
           PROGRESS_FG);
  // update screen
  updateScreen(true);
}
...
/**
 * If the display is not busy, then check if the signal strength has changed, if
 * so, then display it. If the IP address has changed, then display it. If the
 * status has changed, then display it
 *
 * @param force boolean, if true, the screen will be updated regardless of
...
/**
 * If the screen is connected, and the splash screen has not been displayed,
 ...
 * it up into pieces and displays them one at a time
 *
 * @param force if true, the status will be redrawn regardless of whether it's
 * changed or not.
 *
 * @return a boolean value.
 */
bool Display::showStatus(bool force) {
  ...
  if (refresh_status) {
    // clear area
    fillRect(STATUS_AREA_X, STATUS_AREA_Y, STATUS_AREA_W, STATUS_AREA_H,
             SCREEN_BG);
...
bool Display::displaySignal(bool force) {
  ...
  if (refresh_signal || force) {
    ...
    fillRect(SIGNAL_ICON_X, SIGNAL_ICON_Y, SIGNAL_ICON_W, SIGNAL_ICON_H,
             SCREEN_BG);
    if (sig > 0) {
      // Signal %
      s = String(sig);
      ...
      if (sig > 0) {
        fillRect(SIGNAL_ICON_X, SIGNAL_ICON_Y + (SIGNAL_ICON_H * 0.6),
                 SIGNAL_ICON_W_BAR, SIGNAL_ICON_H * 0.4, SIGNAL_FG);
      } else {
        drawRect(SIGNAL_ICON_X, SIGNAL_ICON_Y + (SIGNAL_ICON_H * 0.6),
                 SIGNAL_ICON_W_BAR, SIGNAL_ICON_H * 0.4, SIGNAL_FG);
      }

      if (sig >= 25) {
        fillRect(SIGNAL_ICON_X + SIGNAL_ICON_SPACER_X + SIGNAL_ICON_W_BAR,
                 SIGNAL_ICON_Y + (SIGNAL_ICON_H * 0.4), SIGNAL_ICON_W_BAR,
                 (SIGNAL_ICON_H * 0.6), SIGNAL_FG);
      } else {
        drawRect(SIGNAL_ICON_X + SIGNAL_ICON_SPACER_X + SIGNAL_ICON_W_BAR,
                 SIGNAL_ICON_Y + (SIGNAL_ICON_H * 0.4), SIGNAL_ICON_W_BAR,
                 (SIGNAL_ICON_H * 0.6), SIGNAL_FG);
      }
      ...
      if (sig >= 50) {
        fillRect(
            SIGNAL_ICON_X + (2 * (SIGNAL_ICON_SPACER_X + SIGNAL_ICON_W_BAR)),
            SIGNAL_ICON_Y + (SIGNAL_ICON_H * 0.2), SIGNAL_ICON_W_BAR,
            (SIGNAL_ICON_H * 0.8), SIGNAL_FG);
      } else {
        drawRect(
            SIGNAL_ICON_X + (2 * (SIGNAL_ICON_SPACER_X + SIGNAL_ICON_W_BAR)),
            SIGNAL_ICON_Y + (SIGNAL_ICON_H * 0.2), SIGNAL_ICON_W_BAR,
      ...
      if (sig >= 75) {
        fillRect(
            SIGNAL_ICON_X + (3 * (SIGNAL_ICON_SPACER_X + SIGNAL_ICON_W_BAR)),
            SIGNAL_ICON_Y, SIGNAL_ICON_W_BAR, (SIGNAL_ICON_H), SIGNAL_FG);
      } else {
        drawRect(
            SIGNAL_ICON_X + (3 * (SIGNAL_ICON_SPACER_X + SIGNAL_ICON_W_BAR)),
            SIGNAL_ICON_Y, SIGNAL_ICON_W_BAR, (SIGNAL_ICON_H), SIGNAL_FG);
